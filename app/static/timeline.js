...
const imagesLength = parseInt(timeline.getAttribute('data-images'));
...
function initializeLazyLoading() {
    ...
    const options = {
        ...
        rootMargin: '1500px', // Load images when they're within 1500px of viewport
        threshold: 0.01
    };
    ...
    document.querySelectorAll('.timeline img[data-src]').forEach(img => {
...
function handleTimelineScroll() {
    ...
    // Update scrubber immediately for visual feedback
    const maxScroll = timeline.scrollWidth - timeline.clientWidth;
    const scrollPercentage = maxScroll > 0 ? timeline.scrollLeft / maxScroll : 0;
    scrubberBar.style.left = `calc(${scrollPercentage * 100}% - 8px)`;
    
    // Debounce the actual image display change
